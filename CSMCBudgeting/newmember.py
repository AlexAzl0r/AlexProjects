...
class MembershipUpdate:
    ...
    def format_new_member(self):
        ...
        values = [self.name, self.date_joined,
                  None if self.date_left is None else dt.datetime.strptime(self.date_left, "%Y-%m-%d"),
                  True if self.is_active is None else self.is_active]
...
if __name__ == "__main__":
    job = MembershipUpdate(connection=None,
                           name=args.name,
                           date_joined=args.datejoined,
                           date_left=None if args.dateleft is None else args.dateleft)
    job.run()
